...
public protocol TOTPIssuerMapperServicing: Sendable {
    func lookup(issuer: String) -> IssuerInfo?
}
...
public final class TOTPIssuerMapper: TOTPIssuerMapperServicing {
    ...
    public func lookup(issuer: String) -> IssuerInfo? {
        // Normalize the issuer string
        let normalized = normalizeIssuer(issuer)

        // Try direct lookup
        if let domainInfo = issuerInfos[normalized] {
            return domainInfo
        }

        // Try fuzzy matching
        return fuzzyLookup(normalized: normalized)
    }
}
